...
*
* In case an exception is occurred during processing, the exception handlers
* will be called.
*
...
export class Server extends stream.EventEmitter {
  ...
  */
  #handleMessage (message, streamContext) {
    ...
    try {
      this.#applyHandlers(handlers, request, response, context)
    } catch (err) {
      ...
      this.#applyHandlers(this.#exceptionHandlers, request, response, context,
        err)
    }
  }
  ...
  * @param {any} [error]
  */
  #applyHandlers (handlers, request, response, context, error) {
    for (const handler of handlers) {
      // TODO: Support async handlers
      handler(request, response, context, error)
    ...
    assert(response.isFinished, 'Response not finished!')
  }
}
